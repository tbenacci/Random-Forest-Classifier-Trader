---
title: "One-Week U.S. Equity Prediction Data Prep and Feature Creation"
...
## SPY Holiday Data
```{r}
# assign integer ordered first holding day to last (Buy on Thursday (1) at open, sell on Wednesday (5) at close during standard weeks)
spy_daily <- spy_daily %>%
...
# get unique original (buy week) week_ids in order
week_order <- sort(unique(spy_daily$orig_week_id))
# create a mapping from each original week_id to the next (sell) week_id
...
spy_daily <- spy_daily %>%
  ...
  select(-orig_week_id)
# assign buy and sell prices by the optimal dates that minimize the difference between preferred buy and sell days
spy_daily <- spy_daily %>%
  group_by(week_id) %>%
  mutate(
...
# visualize trading days per week in the sample data
spy_daily %>%
  group_by(week_id) %>%
